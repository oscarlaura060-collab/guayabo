...
import { Truck, Mail, MessageCircle, CheckCircle2 } from "lucide-react";
...
export function PedidoDetalle({
  ...
  const [notificado, setNotificado] = useState(pedido.notificado_envio);
  const [ocupado, setOcupado] = useState(false);

  const colorEstado = estados.find((e) => e.nombre === estado)?.hex ?? undefined;
  const enviado = estado === "Enviado" || !!pedido.guia;

  const datosEnvio = {
    clienteNombre: pedido.cliente_nombre ?? cliente?.nombre ?? null,
  ...
  async function onEstado(nuevo: string) {
    ...
    setOcupado(true);
    const res = await cambiarEstado(pedido.id, nuevo);
    setOcupado(false);
    if (res.ok) {
      setEstado(nuevo);
      toast(`Estado: ${nuevo}`, "exito");
      router.refresh();
    } else {
  ...
  async function guardarEnvio() {
    setOcupado(true);
    const res = await marcarEnviado(pedido.id, { transportadora, guia, url_rastreo: urlRastreo });
    setOcupado(false);
    if (res.ok) {
      setEstado("Enviado");
      setModalEnvio(false);
      toast("Pedido marcado como enviado", "exito");
      router.refresh();
    } else {
      toast(res.error ?? "Error", "error");
    }
  ...
  async function enviarCorreo() {
    setOcupado(true);
    const res = await notificarEnvioEmail(pedido.id);
    setOcupado(false);
    if (res.ok) {
      setNotificado(true);
      toast("Correo de aviso enviado", "exito");
    } else {
  ...
  function avisarWhatsapp() {
    if (!waUrl) {
      toast("El cliente no tiene WhatsApp registrado", "error");
      return;
    }
    window.open(waUrl, "_blank", "noopener,noreferrer");
    marcarNotificado(pedido.id).then(() => setNotificado(true));
  }
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm" style={{ color: "var(--tenue)" }}>
              {pedido.cliente_nombre ?? cliente?.nombre ?? "Sin cliente"} · {fmtFecha(pedido.fecha)}
              {pedido.fecha_entrega && ` · entrega ${fmtFecha(pedido.fecha_entrega)}`}
            </p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <StatusChip texto={estado} color={colorEstado} />
            <StatusChip texto={pedido.est_pago} color={colorPago[pedido.est_pago]} />
          </div>
        </div>
        ...
        {puedeEscribir && (
          <div className="mt-4 flex flex-wrap items-center gap-3">
            ...
            <Button variante="contorno" onClick={() => setModalEnvio(true)} disabled={ocupado}>
              <Truck size={16} /> {enviado ? "Editar envío" : "Marcar enviado"}
            </Button>
          </div>
        )}
      </div>
      ...
      {enviado && (
        <div className="gy-card p-4">
          <div className="mb-2 flex items-center gap-2 text-sm font-semibold">
            ...
            {notificado && (
              <span className="inline-flex items-center gap-1 text-xs" style={{ color: "#3AA76D" }}>
                <CheckCircle2 size={14} /> avisado
              </span>
            )}
          </div>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
            <Dato etiqueta="Transportadora" valor={pedido.transportadora} />
            <Dato etiqueta="Guía" valor={pedido.guia} />
            {pedido.fecha_envio && <Dato etiqueta="Enviado" valor={fechaHora(pedido.fecha_envio)} />}
          </div>
          {puedeEscribir && (
            <div className="mt-3 flex flex-wrap gap-2">
              <Button variante="contorno" onClick={enviarCorreo} disabled={ocupado}>
                <Mail size={16} /> Avisar por correo
              </Button>
              <button
                className="gy-btn gy-btn-solido"
          ...
          {puedeEscribir && !cliente?.email && (
            <p className="mt-2 text-xs" style={{ color: "var(--tenue)" }}>
              El cliente no tiene correo; usa WhatsApp o agrega el correo en su ficha.
            </p>
          )}
        </div>
      )}
      ...
      <div className="gy-card overflow-hidden">
        <div className="gy-table-wrap">
          <table className="gy-table">
            <thead>
              <tr><th>Prenda</th><th>Cant.</th><th>Precio</th><th>Total</th></tr>
            </thead>
            <tbody>
              {items.map((it) => (
                <tr key={it.id}>
                  <td>
                    {it.nombre}
                    <span className="ml-1 text-xs" style={{ color: "var(--tenue)" }}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="gy-card p-4">
          ...
          <div className="flex flex-col gap-1 text-sm">
            <Linea etiqueta="Subtotal" valor={pesos(pedido.subtotal)} />
            <Linea etiqueta="Descuento" valor={pesos(pedido.descuento)} />
            <Linea etiqueta="Envío" valor={pesos(pedido.envio)} />
            <div className="mt-1 flex justify-between border-t pt-2 font-semibold" style={{ borderColor: "var(--borde-suave)" }}>
              <span>Total</span><span className="gy-cifra text-lg">{pesos(pedido.total)}</span>
            </div>
            <Linea etiqueta="Pagado" valor={pesos(pedido.pagado)} />
            <Linea etiqueta="Saldo" valor={pesos(pedido.saldo)} />
          </div>
        </div>

        <div className="gy-card p-4">
          <div className="mb-2 text-sm font-semibold">Pagos</div>
          {pagos.length === 0 ? (
      ...
      <Modal abierto={modalEnvio} onClose={() => setModalEnvio(false)} titulo="Datos de envío">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={guardarEnvio} disabled={ocupado || !transportadora.trim() || !guia.trim()}>
              {ocupado ? "Guardando…" : "Marcar enviado"}
            </Button>
          </div>
        </div>
      </Modal>
...
function Linea({ etiqueta, valor }: { etiqueta: string; valor: string }) {
  return (
    <div className="flex justify-between">
      <span style={{ color: "var(--tenue)" }}>{etiqueta}</span>
