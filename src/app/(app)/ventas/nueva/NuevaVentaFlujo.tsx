...
export function NuevaVentaFlujo({
  ...
  function agregarPrenda(p: PrendaOpt) {
    setCart((cs) => {
      ...
      if (i >= 0) {
        return cs.map((x, idx) => (idx === i ? { ...x, cantidad: x.cantidad + 1 } : x));
      }
      return [
        ...cs,
        {
  ...
  function setCantidad(i: number, delta: number) {
    setCart((cs) =>
      cs.map((x, idx) => (idx === i ? { ...x, cantidad: Math.max(1, x.cantidad + delta) } : x)),
    );
  }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_340px]">
      {/* Columna izquierda: cliente + prendas */}
      <div className="flex flex-col gap-4">
        ...
        <div className="gy-card p-4">
          ...
          <div className="relative">
            ...
            {mostrarClientes && clientesFiltrados.length > 0 && !clienteId && (
              <div
                className="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border bg-[var(--color-tarjeta)] shadow-lg"
              ...
              >
                {clientesFiltrados.map((c) => (
                  <button
                    key={c.id}
                    type="button"
                    className="flex w-full items-center justify-between gap-2 px-3 py-2 text-left text-sm hover:bg-[var(--color-fondo)]"
                    onClick={() => {
                      setClienteId(c.id);
          ...
          {!clienteId && clienteQuery && (
            <p className="mt-2 text-xs" style={{ color: "var(--tenue)" }}>
              Se registrará como “{clienteQuery}” sin ficha de cliente.
            </p>
          )}
        </div>
        ...
        <div className="gy-card p-4">
          ...
          <div className="relative">
            <label className="relative flex items-center">
              ...
              <input
                ...
                value={prendaQuery}
                onChange={(e) => setPrendaQuery(e.target.value)}
              />
            </label>
          ...
          <div className="mt-3 flex flex-col gap-2">
            {cart.length === 0 && (
              <p className="py-4 text-center text-sm" style={{ color: "var(--tenue)" }}>
                Busca y agrega prendas a la venta.
              </p>
            )}
            {cart.map((it, i) => {
                ...
                >
                  <div className="min-w-32 flex-1">
                    ...
                    <div className="text-xs" style={{ color: "var(--tenue)" }}>
                      ...
                      {excede && (
                        <span style={{ color: "#D33A2C" }}> · supera stock ({it.stock})</span>
                      )}
                    </div>
                  </div>
                  <input
                  ...
                  <button
                    ...
                    onClick={() => setCart((cs) => cs.filter((_, idx) => idx !== i))}
                    aria-label="Quitar"
                  >
                    <Trash2 size={15} />
                  </button>
      ...
      <div className="flex flex-col gap-4 lg:sticky lg:top-20 lg:self-start">
        ...
        <div className="gy-card p-4">
          ...
          <div className="flex flex-col gap-2 text-sm">
            <Linea etiqueta="Subtotal" valor={pesos(subtotal)} />
        ...
        <div className="gy-card p-4">
          ...
          {pago > 0 && saldo > 0 && (
            <p className="mt-2 text-xs" style={{ color: "#9a6f0a" }}>Queda un saldo de {pesos(saldo)} (abono).</p>
          )}
          {pago > total && (
            <p className="mt-2 text-xs" style={{ color: "#D33A2C" }}>El pago supera el total.</p>
          )}
          {sinStock && (
            <p className="mt-2 text-xs" style={{ color: "#D33A2C" }}>
              Hay prendas sin stock suficiente. Actívalo en Configuración (venta bajo pedido) o ajusta la cantidad.
            </p>
          )}
